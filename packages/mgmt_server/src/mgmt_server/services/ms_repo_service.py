"""ModelScope repository management service for read, delete, and cache-status operations.

Mirrors ``repo_service.py`` but operates on the ``ms_repo_*`` cache tables and
S3 ``ms/`` blob prefix. HF code is untouched (Decision A: parallel tables).
"""
...
from database.db_models import MsRepoProfile, MsRepoSnapshot, MsRepoTreeItem, RepoStatus
...
from storage import build_ms_blob_key, build_ms_blob_prefix, s3_client
...
class MsRepoService:
    """Service for ModelScope repository lifecycle management."""

    def __init__(self, session: AsyncSession, task_service: TaskService) -> None:
        self._session = session
        self._task_service = task_service
        self._profile_repo = MsRepoProfileRepository(session)
        self._snapshot_repo = MsRepoSnapshotRepository(session)
        self._tree_repo = MsRepoTreeRepository(session)
    ...
    async def get_profile_by_repo_id(self, repo_id: str) -> MsRepoProfile | None:
    ...
    async def _delete_blobs_from_ids(
        ...
        async def _delete_one(blob_id: str) -> bool:
            async with semaphore:
                s3_key = build_ms_blob_key(repo_id, repo_type, blob_id)
                try:
    ...
    async def _delete_untracked_repo(
        ...
        """
        prefix = build_ms_blob_prefix(repo_id, repo_type)
