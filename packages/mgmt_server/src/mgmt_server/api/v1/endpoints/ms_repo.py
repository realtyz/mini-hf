"""ModelScope repository management endpoints.

Mirrors ``repo.py`` (HF) but operates on the ``ms_repo_*`` cache tables via
``MsRepoService``. Reuses the HF response schemas (``RepoListResponse`` /
``RepoDetailResponse`` / ``RepoTreeResponse`` etc.) since ``MsRepo*`` models
expose the same fields. HF code is untouched (Decision A: parallel tables).
"""
...
from database.db_models import MsRepoProfile, MsRepoSnapshot, RepoStatus
from database.db_repositories.ms_repo_snapshot import SizeStats
from mgmt_server.api.deps import AdminUserDep, CacheScanServiceDep, MsRepoServiceDep
...
class _MsRepoDetailProvider(Protocol):
    ...
    async def get_repo_detail(
        ...
        repo_id: str,
        repo_type: str,
    ) -> tuple[MsRepoProfile | None, list[MsRepoSnapshot], dict[str, SizeStats]]: ...
...
async def list_repositories(
    ...
    """List ModelScope repositories with filtering, search, sorting and pagination."""
...
async def list_public_repositories(
    ...
    """List publicly visible ModelScope repositories (default: active and updating only)."""
...
async def _get_repo_detail(
    repo_id: str,
    repo_type: str,
    repo_service: _MsRepoDetailProvider,
) -> RepoDetailResponse:
...
async def get_model_detail(
    ...
    """Get ModelScope model detail with profile and snapshots."""
...
async def get_dataset_detail(
    ...
    """Get ModelScope dataset detail with profile and snapshots."""
...
async def delete_repository(
    ...
    """Delete an entire cached ModelScope repository and all associated records.
...
async def get_file_download(
    ...
    Public endpoint (no auth required): compatible with ModelScope download flow.
    """
...
async def get_repo_tree(
    ...
    """Get ModelScope repository tree (files and directories) for a specific commit."""
