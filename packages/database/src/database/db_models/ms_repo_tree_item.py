...
class MsRepoTreeItem(Base):
    """Repository tree item (file or folder).

    Stores the content of list_repo_tree() results.
    """

    __tablename__ = "ms_repo_tree_items"
    __table_args__ = (
        UniqueConstraint(
            "commit_hash",
            "path",
            name="uq_ms_repo_tree_items_commit_path",
        ),
        # 单字段索引：commit_hash
        Index(
    ...
    lfs_size: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    lfs_pointer_size: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
