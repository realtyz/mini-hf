...
class MsRepoProfile(Base):
    """Repository snapshot (commit) information."""

    __tablename__ = "ms_repo_profiles"
    __table_args__ = (
        # 复合唯一索引：最高频查询 (repo_id + repo_type)
        Index(
            "idx_ms_repo_profiles_repo_id_repo_type", "repo_id", "repo_type", unique=True
        ),
        # 单列索引：delete_repository 端点只使用 repo_id 查询
        Index("idx_ms_repo_profiles_repo_id", "repo_id"),
        # 复合索引：状态过滤 + 时间排序（用于列表查询）
        Index("idx_ms_repo_profiles_status_updated_at", "status", "cache_updated_at"),
        {"schema": "mini_hf"},
    )
