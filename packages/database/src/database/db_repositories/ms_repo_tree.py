"""Repository for ModelScope repository tree item operations."""
...
class MsRepoTreeRepository:
    """Repository for MsRepoTreeItem entity operations.
    ...
    async def batch_insert_items(
        ...
        Args:
            ...
            items: List of item dicts with keys: path, item_type, size, oid, lfs_oid, lfs_size, lfs_pointer_size
    ...
    async def get_file_tree(
        ...
        """
        stmt = (
            select(MsRepoTreeItem)
            .where(MsRepoTreeItem.commit_hash == commit_hash)
            .order_by(MsRepoTreeItem.type.asc(), MsRepoTreeItem.path.asc())
        )
        result = await self._session.execute(stmt)
    ...
    async def get_file_tree_paginated(
        ...
        stmt = (
            ...
            .order_by(
                ...
                MsRepoTreeItem.type.asc(),
                MsRepoTreeItem.path.asc(),
            )
            .offset(offset)
            .limit(page_size)
        )
    ...
    async def get_tree_by_cursor(
        ...
        stmt = (
            select(MsRepoTreeItem)
            .where(MsRepoTreeItem.commit_hash == commit_hash)
            .order_by(MsRepoTreeItem.path.asc())
        )
        if cursor_path is not None:
            stmt = stmt.where(MsRepoTreeItem.path > cursor_path)
    ...
    async def set_item_cached(
        ...
        """
        stmt = (
            update(MsRepoTreeItem)
            .where(
                MsRepoTreeItem.commit_hash == commit_hash,
                MsRepoTreeItem.path == path,
            )
    ...
    async def get_blob_ids_by_snapshot(
        ...
        """
        stmt = select(
            MsRepoTreeItem.lfs_oid,
            MsRepoTreeItem.oid,
        ).where(
            MsRepoTreeItem.commit_hash == commit_hash,
            MsRepoTreeItem.type == TreeItemType.FILE,
        )
        result = await self._session.execute(stmt)
    ...
    async def blob_exists_in_other_active_commits(
        ...
        stmt = (
            select(func.count())
            .select_from(MsRepoTreeItem)
            .join(
                MsRepoSnapshot,
                MsRepoSnapshot.commit_hash == MsRepoTreeItem.commit_hash,
            )
            .where(
                MsRepoSnapshot.repo_id == repo_id,
                MsRepoSnapshot.commit_hash != exclude_commit_hash,
                MsRepoSnapshot.status == SnapshotStatus.ACTIVE,
                MsRepoTreeItem.type == TreeItemType.FILE,
                or_(
                    MsRepoTreeItem.oid == blob_id,
                    MsRepoTreeItem.lfs_oid == blob_id,
                ),
            )
        )
        result = await self._session.execute(stmt)
    ...
    async def get_cached_files_count(
        ...
        cached_stmt = select(func.count()).where(
            MsRepoTreeItem.commit_hash == commit_hash,
            MsRepoTreeItem.type == TreeItemType.FILE,
            MsRepoTreeItem.is_cached == True,  # noqa: E712
        )
    ...
    async def get_tree_item_by_path(
        ...
        """
        stmt = select(MsRepoTreeItem).where(
            MsRepoTreeItem.commit_hash == commit_hash,
            MsRepoTreeItem.path == path,
        )
        result = await self._session.execute(stmt)
    ...
    async def get_cached_paths(
        ...
        """
        stmt = select(MsRepoTreeItem.path).where(
            MsRepoTreeItem.commit_hash == commit_hash,
            MsRepoTreeItem.type == TreeItemType.FILE,
            MsRepoTreeItem.is_cached == True,  # noqa: E712
        )
