...
export const SOURCE_LABELS: Record<RepoSource, string> = {
...
export function getSourceLabel(source: string): string {
  return SOURCE_LABELS[source as RepoSource] ?? source;
}

export function getRepoTypeLabel(repoType: string): string {
  return REPO_TYPE_LABELS[repoType as RepoType] ?? repoType;
}
