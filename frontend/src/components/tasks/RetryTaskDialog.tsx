...
export function RetryTaskDialog({
  ...
  // Computed stats based on current selection + cached files
  const selectedStats = useMemo(() => {
    ...
    for (const item of previewData?.items ?? []) {
      ...
      if (cachedSet.has(item.path) || effectiveSelected.has(item.path)) {
        count++
        size += item.size
      }
    }
      ...
      <DialogContent className="min-w-2xl max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <AnimatePresence mode="wait">
            {isStep === 'loading' ? (
              <div key="loading" className="h-full flex items-center justify-center py-16">
              ...
              <div key="error" className="h-full flex flex-col items-center justify-center py-16 px-6">
              ...
              <div key="all-cached" className="h-full flex flex-col">
                <div className="flex flex-col items-center justify-center py-12 px-6">
                  ...
                  {previewData?.cached_commit_hash && (
                    <code className="font-mono text-xs bg-muted px-3 py-1.5 rounded text-muted-foreground">
              ...
              <div key="preview" className="h-full flex flex-col">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1.5">
                              <span className="font-medium text-foreground">{selectedStats.count}</span>
                              个文件将下载
                            </span>
                            <span className="text-border">·</span>
                            <span className="flex items-center gap-1.5">
                              <span className="font-medium text-foreground">{formatBytes(selectedStats.size)}</span>
                              所需空间
                            </span>
                            {nonCachedCount > 0 && (
                              <>
                                <span className="text-border">·</span>
                                <span>{nonCachedCount} 个文件未缓存</span>
                              </>
                            )}
                          </div>
                        </div>
