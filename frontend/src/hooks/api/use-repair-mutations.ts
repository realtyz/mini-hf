...
export function useSetProfileStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      repoId,
      ...body
    }: SetProfileStatusRequest & { repoId: string }): Promise<RepairResponse> => {
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.repos.detail(variables.repoId) })
...
export function useSetSnapshotStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      repoId,
      ...body
    }: SetSnapshotStatusRequest & { snapshotId: number; repoId: string }): Promise<RepairResponse> => {
      return api.patch<RepairResponse>(
