...
function AnnouncementListItem({
  ...
  const typeConfig =
    announcementTypeConfig[item.announcement_type] ??
...
export const AnnouncementTab = memo(function AnnouncementTab() {
  ...
  const [initialForm, setInitialForm] =
    useState<AnnouncementCreateRequest>(emptyForm);
  ...
  const setField = useCallback(
    <K extends keyof AnnouncementCreateRequest>(
      key: K,
      value: AnnouncementCreateRequest[K],
    ) => {
  ...
  const handleSave = useCallback(async () => {
    if (editingId !== null) {
      await updateMutation.mutateAsync({ id: editingId, ...form } as {
        id: number;
      } & AnnouncementUpdateRequest);
      toast.success("公告已更新");
    } else {
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setField(
                    "announcement_type",
                    v as AnnouncementItem["announcement_type"],
                  )
                }
              >
                <SelectTrigger>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              确定要删除公告「{deleteTarget?.title || "无标题"}
              」吗？此操作不可撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
