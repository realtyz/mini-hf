...
export const ConfigField = memo(function ConfigField({ field, value, form, onChange }: ConfigFieldProps) {
  const id = `config-${field.key}`
  const className = cn(field.ui.col_span === 2 && 'sm:col-span-2')

  if (field.ui.widget === 'hf_endpoint_list') {
    return <HfEndpointListField className={className} field={field} value={value} form={form} onChange={onChange} />
  }
    ...
    <div className={cn('space-y-1.5', className)}>
      ...
      {field.ui.widget === 'switch' ? (
        ...
        <Input
          ...
          placeholder={field.sensitive && field.has_value ? '已设置，留空保持不变' : field.ui.placeholder}
          className="transition-all duration-200 focus-visible:ring-primary/20"
          onChange={(event) => onChange(field.key, event.target.value)}
        />
      )}
      ...
      {field.ui.helper_text ? (
        <p className="text-xs text-muted-foreground/80">{field.ui.helper_text}</p>
      ) : null}
    </div>
  )
})
